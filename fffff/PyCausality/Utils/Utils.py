    ...
    def _compute_covariance(self, covar):
        ...
        if covar is not None:
            ...
            try:
                ...
                self._data_inv_cov = linalg.inv(self._data_covariance + np.identity(len(self._data_covariance))*0.00001)
...
def pdf_kde(df, gridpoints=None, bandwidth=1, covar=None):
    ...
    for i in range(df.shape[1]):
        slices.append(slice(np.amin(df, axis=0)[i], np.amax(df, axis=0)[i], N))
